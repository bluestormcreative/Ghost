...
describe("Posts Content API", function () {
    ...
    it("Can request posts", async function () {
        ...
        assert.equal(
            res.body.posts[0].slug,
            "welcome",
            'Default order "published_at desc" check'
        );
        assert.equal(
            res.body.posts[6].slug,
            "integrations",
            'Default order "published_at desc" check'
        );
        ...
        // kitchen sink
        assert.equal(
            res.body.posts[9].slug,
            fixtureManager.get("posts", 1).slug
        );
        ...
        assert.match(
            res.body.posts[7].html,
            /<a href="http:\/\/127.0.0.1:2369\/about#nowhere" title="Relative URL/
        );
    ...
    it("Can filter posts by authors", async function () {
        const res = await agent
            .get(
                "posts/?filter=authors:[joe-bloggs,pat,ghost,slimer-mcectoplasm]&include=authors"
            )
            .expectStatus(200)
            .matchHeaderSnapshot({
                "content-version": anyContentVersion,
    ...
    it("Can include relations", async function () {
        await agent
            ...
            .matchBodySnapshot({
                posts: new Array(11).fill(postMatcherShallowIncludes),
            });
    });
    ...
    it("Can filter by published date", async function () {
        ...
        const publishedAt = moment(post.published_at).format(
            "YYYY-MM-DD HH:mm:ss"
        );
        ...
        const publishedAt2 = moment(post2.published_at).format(
            "YYYY-MM-DD HH:mm:ss"
        );
    ...
    it("Can use post reading time as field", async function () {
        await agent
    ...
    it("Adds ?ref tags", async function () {
                ...
                mobiledoc: JSON.stringify({
                    ...
                    cards: [
                        [
                            "html",
                            {
                                html: '<a href="https://example.com">Link</a><a href="invalid">Test</a>',
                            },
                        ],
                    ],
                    markups: [],
        ...
        assert(
            ...
            "Html not expected (should contain ?ref): " +
                response.body.posts[0].html
        );
